@php
   ...
   $headers = [
           $name['name_'.App::getLocale()] => route('admin.product.index', App::getLocale()),
...
{{--                                    <a href="{{ route($resource['route'].'.edit', [App::getLocale(), $item->id]) }}"><i class="fa fa-edit left-icon"></i></a>--}}
                        ...
                        @include('dashboard.components.dangerModalImage', ['user_name' => $item->name, 'id' => $item->id, 'value' => $value, 'resource' => $resource['route']])
                    @endforeach<br>
                </div>
                {!! Form::close()!!}
                @endif
